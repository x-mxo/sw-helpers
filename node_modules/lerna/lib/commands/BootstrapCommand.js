  ...
  _createClass(BootstrapCommand, [{
    ...
    value: function execute(callback) {
      ...
      this.bootstrapPackages(function (err) {
        if (err) {
          ...
          _this3.logger.success(`Successfully bootstrapped ${_this3.filteredPackages.length} packages.`);
          callback(null, true);
        }
      });
    }
    ...
    value: function getDependenciesToInstall() {
      ...
      var findPackage = function findPackage(name, version) {
        return (0, _find2.default)(_this6.packages, function (pkg) {
      ...
      Object.keys(this.repository.package.allDependencies).forEach(function (name) {
        var version = _this6.repository.package.allDependencies[name];
        depsToInstall[name] = {
          versions: { [version]: 0 },
          dependents: { [version]: [] }
        };
      ...
      packages.forEach(function (pkg) {
        ...
        .filter(function (dep) {
          return !hasPackage(dep.name, dep.version) || !pkg.hasMatchingDependency(dep, _this6.logger);
      ...
      Object.keys(depsToInstall).forEach(function (name) {
        ...
        if (hoist && _PackageUtilities2.default.isHoistedPackage(name, hoist, nohoist)) {
          ...
          rootVersion = _this6.repository.package.allDependencies[name] || commonVersion;

          if (rootVersion !== commonVersion) {
            _this6.logger.warn(`The repository root depends on ${name}@${rootVersion}, ` + `which differs from the more common ${name}@${commonVersion}.`);
          }
          ...
          root.push({
            ...
            dependents: (dependents[rootVersion] || []).map(function (dep) {
              return _this6.packageGraph.get(dep).package;
            }),
            dependency: `${name}@${rootVersion}`,
            isSatisfied: _this6.repository.hasDependencyInstalled(name, rootVersion)
          });
        }
        ...
        Object.keys(versions).forEach(function (version) {
          ...
          dependents[version].forEach(function (pkg) {
            if (rootVersion) {
              _this6.logger.warn(`"${pkg}" package depends on ${name}@${version}, ` + `which differs from the hoisted ${name}@${rootVersion}.`);
            }
    ...
    value: function installExternalDependencies(callback) {
      ...
      if (Object.keys(root).length) {
        ...
        actions.push(function (cb) {
          if (depsToInstallInRoot.length) {
            _this7.logger.info("Installing hoisted dependencies into root");
          }

          _NpmUtilities2.default.installInDir(_this7.repository.rootPath, depsToInstallInRoot, _this7.npmConfig, function (err) {
            ...
            _async2.default.series(root.map(function (_ref5) {
              ...
              return function (cb) {
                var _ref6 = _this7.hoistedPackageJson(name) || {},
                    bin = _ref6.bin;

                if (bin) {
                  _async2.default.series(dependents.map(function (pkg) {
                    return function (cb) {
                      var src = _this7.hoistedDirectory(name);
                      var dest = pkg.nodeModulesLocation;
                      _this7.createBinaryLink(src, dest, name, bin, cb);
                    };
                  }), cb);
                } else {
                  cb();
              ...
              _this7.progressBar.tick("Install hoisted");
              cb(err);
            });
          });
        });
        ...
        actions.push(function (cb) {
          _this7.logger.info("Pruning hoisted dependencies");
          ...
          _async2.default.series(root.map(function (_ref7) {
            ...
            return function (cb) {
              var dirs = dependents.filter(function (pkg) {
                return pkg.nodeModulesLocation !== _this7.repository.nodeModulesLocation;
              }).map(function (pkg) {
                return _path2.default.join(pkg.nodeModulesLocation, name);
              });

              _FileSystemUtilities2.default.rimraf(dirs, cb);
            };
          }), function (err) {
            _this7.progressBar.tick("Prune hoisted");
            cb(err);
          });
        });
      }
      ...
      Object.keys(leaves).map(function (pkgName) {
        return { pkg: _this7.packageGraph.get(pkgName).package, deps: leaves[pkgName] };
        ...
        if (deps.some(function (_ref9) {
          ...
          actions.push(function (cb) {
            _NpmUtilities2.default.installInDir(pkg.location, deps.map(function (_ref10) {
              ...
              return dependency;
            }), _this7.npmConfig, function (err) {
              _this7.progressBar.tick(pkg.name);
              cb(err);
            });
          });
        }
    ...
    value: function symlinkPackages(callback) {
      ...
      this.filteredPackages.forEach(function (filteredPackage) {
        ...
        Object.keys(filteredPackage.allDependencies).filter(function (dependency) {
          ...
          var match = _this8.packageGraph.get(dependency);
          return match && filteredPackage.hasMatchingDependency(match.package);
        }).forEach(function (dependency) {
          // get Package of dependency
          var dependencyPackage = _this8.packageGraph.get(dependency).package;
          ...
          if (!_FileSystemUtilities2.default.existsSync(dependencyPackageJsonLocation)) {
            _this8.logger.error(`Unable to find package.json for ${dependency} dependency of ${filteredPackage.name},  ` + "Skipping...");
          } else {
            ...
            if (_FileSystemUtilities2.default.existsSync(pkgDependencyLocation)) {
              ...
              if (isDepSymlink !== false && isDepSymlink !== dependencyLocation) {
                _this8.logger.warn(`Symlink already exists for ${dependency} dependency of ${filteredPackage.name}, ` + "but links to different location. Replacing with updated symlink...");
                // installed dependency is not a symlink
              } else if (isDepSymlink === false) {
                _this8.logger.warn(`${dependency} is already installed for ${filteredPackage.name}. ` + "Replacing with symlink...");
            ...
            if (dependencyPackageJson.bin) {
              ...
              packageActions.push(function (cb) {
                _this8.createBinaryLink(dependencyLocation, destFolder, dependency, dependencyPackageJson.bin, cb);
              });
            }
          }
        });
        ...
        actions.push(function (cb) {
          _async2.default.series(packageActions, function (err) {
            _this8.progressBar.tick(filteredPackage.name);
            cb(err);
          });
        });
      });
